...
- **Backup/histórico**: ativar **DynamoDB Point-in-Time Recovery (PITR)** (restaura a tabela inteira para qualquer instante dos últimos 35 dias — protege contra erro do próprio autor, já que é admin único sem revisão por terceiros). Complementar com uma export periódica (ex.: diária, via Lambda agendada) da tabela para S3 em JSON, como backup adicional de baixíssimo custo.
